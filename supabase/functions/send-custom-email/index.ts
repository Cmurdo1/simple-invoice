...
const DEFAULT_FROM = "Honest Invoice <noreply@honestinvoice.com>";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { to, subject, body, from, replyTo, cc, bcc } = await req.json();
    if (!to || !subject || !body) {
    ...
    console.log(`[SEND-CUSTOM-EMAIL] ${recipients.join(", ")} | id: ${data?.id}`);
